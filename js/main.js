...
class App {
  ...
  runCommand(key, canvasPos) {
    ...
    if (key === 'duplicateStream') {
      ...
      if (selIds.length === 1) levelIt(this, tab, selIds[0]);
    ...
    box.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 16px;">
        <div>
          ...
          <div class="prop-row checkbox" id="rm-align-section-row" title="On by default. Between two parts connected by a real edge that share the same Section (e.g. a Business Organization Unit and a Function it's assigned to), prioritizes placing them in the same column, one row apart — nearby (packed shelf-adjacent) for Centralize in Clusters instead. Not available for Force-directed."><input type="checkbox" id="rm-align-section" ${rOpt('alignBySection') === false ? '' : 'checked'} /><label for="rm-align-section" id="rm-align-section-label">Align by section</label></div>
