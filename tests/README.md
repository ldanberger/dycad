...
| `check_cubeorder_covers_all_elements` | A real near-miss caught while adding a new element type (BusinessEvent): `cubeOrder` is a second, hand-maintained list documented as covering every element type in `settings.elements`, with nothing else enforcing that — a new element left out of it would silently fall through to the 3D View's defensive tkDisplayOrder/alphabetical fallback instead of its intended position. Checks both directions: every element appears in cubeOrder exactly once, no orphaned cubeOrder entries, no duplicates |
