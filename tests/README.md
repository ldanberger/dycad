...
| `check_view3d_boots` | Catalogs > 3D View failing to open, opening more than one tab instead of a singleton, any console error while lazy-loading the vendored Three.js/OrbitControls, or (a real bug found while building this) the "Loading 3D view..." placeholder never being cleared before the actual `<canvas>` is appended, leaving it pushed out of view behind the placeholder |
